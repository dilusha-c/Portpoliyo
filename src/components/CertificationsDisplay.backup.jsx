...
const CertificationsDisplay = ({ theme }) => {
  ...
  // Filter certificates based on whether to show all or just featured ones
  ...
  return (
    <div className="relative">
      {/* Robot animation positioned to the side */}
      <motion.div 
      ...
      >
        ...
        {/* Speech bubble that appears when a certificate is selected */}
        {selectedCert && (
          <motion.div 
            className={`absolute -left-56 md:-left-40 top-10 w-52 md:w-64 p-4 rounded-lg ${
              theme === 'dark' ? 'bg-slate-700' : 'bg-white'
            } shadow-lg ${roboticMode ? 'tech-panel border-cyan-500 border' : ''}`}
          ...
          >
            ...
            {roboticMode ? (
              <>
                <p className="text-sm text-cyan-400 font-mono font-bold mb-2">
                  <span className="inline-block w-2 h-2 bg-cyan-400 mr-1 animate-pulse rounded-full"></span>
                  CERT_ANALYSIS_COMPLETE
                </p>
                <p className="text-xs font-mono text-cyan-200 leading-tight">
        ...
        {/* Show robot animation reaction when a certificate is clicked */}
        {robotAnimating && (
          <motion.div 
      ...
      {roboticMode ? (
        /* Static grid for robotic mode - no animations */
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {displayedCerts.map((cert, index) => (
            <div 
              ...
              } robot-border overflow-hidden data-stream cursor-pointer`}
              onClick={() => handleCertClick(cert, index)}
            >
              <span /> {/* This is for the robot-border corners */}
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-xl font-bold">{cert.title}</h3>
                <div className={`${theme === 'dark' ? 'bg-cyan-500/20' : 'bg-blue-500/20'} p-2 rounded-full`}>
      ...
      <div className="flex justify-center mt-12">
        <motion.button
          ...
          } ${roboticMode ? 'robot-border tech-panel relative overflow-hidden' : ''} transition-colors shadow-md`}
        ...
        >
          {roboticMode && (
            <div className="absolute inset-0 pointer-events-none">
              <div className="absolute top-0 left-0 w-2 h-2">
                <div className="w-1 h-[1px] bg-cyan-400"></div>
                <div className="w-[1px] h-1 bg-cyan-400"></div>
              </div>
              <div className="absolute top-0 right-0 w-2 h-2">
                <div className="w-1 h-[1px] bg-cyan-400 ml-1"></div>
                <div className="w-[1px] h-1 bg-cyan-400 ml-1"></div>
              </div>
              <div className="absolute bottom-0 left-0 w-2 h-2">
                <div className="w-1 h-[1px] bg-cyan-400 mt-1"></div>
                <div className="w-[1px] h-1 bg-cyan-400 mt-1"></div>
              </div>
              <div className="absolute bottom-0 right-0 w-2 h-2">
                <div className="w-1 h-[1px] bg-cyan-400 ml-1 mt-1"></div>
                <div className="w-[1px] h-1 bg-cyan-400 ml-1 mt-1"></div>
              </div>
            </div>
          )}
          ...
          {roboticMode && (
            <div className="absolute inset-y-0 right-0 w-1 bg-cyan-400/20 overflow-hidden">
